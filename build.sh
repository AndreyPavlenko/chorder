...
create_source_packages() {
    ...
    eval local $(grep -m 1 '^VERSION' chorder)
    ...
    do
        eval local $(git --git-dir="$DIR/.git" show ${h}:chorder | \
                     grep -m 1 '^VERSION' chorder || echo VERSION='1.0.0')
        ver_replace="$ver_replace; s/#VER#-#CIn#~${h}/${VERSION}-${c}~${h}/"
        c=$(( $c + 1 ))
    done
